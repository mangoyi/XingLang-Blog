...
Base.prototype.index = function(){
	var children = this.elements[0].parentNode.children; //得到父节点 ul list,再.children 是一个子节点
	// alert(children.length)  //求出这个集合的长度
...
Base.prototype.center = function (width, height) {
	...
	var top = (getInner().height - height) / 2  + getScroll().top;  //加上滚动条的距离
...
Base.prototype.lock = function () {
	for (var i = 0; i < this.elements.length; i ++) {
		...
		this.elements[i].style.width = getInner().width +getScroll().left + 'px';  //所有位置全部遮罩
		this.elements[i].style.height = getInner().height + getScroll().top +'px';
		this.elements[i].style.display = 'block';
		document.documentElement.style.overflow = 'hidden';
		// addEvent(window, 'scroll', scrollTop);

		//addEvent(document,'mousedown',predef);     //47解决关闭大图无法拖动问题
		//addEvent(document,'mouseup',predef);
		//addEvent(document,'selectstart',predef);  //又同时导致无法输入内容
...
Base.prototype.unlock = function () {
	for (var i = 0; i < this.elements.length; i ++) {
		...
		// removeEvent(document,'mousedown',predef);  //47解决大图无法拖动问题
		// removeEvent(document,'mouseup',predef);
		// removeEvent(document,'selectstart',predef);
		removeEvent(this.elements[i],'mousedown',predef);  //47解决大图无法拖动问题
...
Base.prototype.resize = function (fn) {
	for (var i = 0; i < this.elements.length; i ++) {
		...
		addEvent(window, 'resize', function () {
			fn(); //+getScroll().left 为 了解决ie的问题 47中的问题
			if (element.offsetLeft > getInner().width +getScroll().left - element.offsetWidth) {
				...
				if(element.offsetLeft <= 0 + getScroll().left){
					element.style.left = 0 + getScroll().left + 'px';  //49的问题
				}
			...
			if(element.offsetTop > getInner().height +getScroll().top- element.offsetHeight) {
				element.style.top = getInner().height +getScroll().top- element.offsetHeight + 'px';
				 if(element.offsetTop <= 0 + getScroll().top){
					element.style.top = 0 + getScroll().top + 'px';  //49的问题
				}
			}
		});
	}
			...
			for (var i in mul) {
				...
				if (attr == 'opacity') {
					...
					if (step == 0) {
						...
						var s;
						if((/msie ([\d.]+)/).test(ua)){  //判断浏览器为ie
							// element.style[attr] = parseInt(getStyle(element, attr)) + step;
						}else{

							element.style[attr] = parseInt(getStyle(element, attr)) + step + 'px';  //google
						}
